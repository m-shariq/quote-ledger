...
add_purchase = function (party, date, callback) {
  db.connection.serialize(function () {
    var stmt = db.connection.prepare(
      "INSERT INTO payment (party_id,date,status) VALUES (?,?,?)"
    );
...
add_purchase_item = function (bank, comment, amount, purchase_id,city_id, callback) {
  db.connection.serialize(function () {
    var stmt = db.connection.prepare(
      "INSERT INTO payment_item (bank,comment,amount,payment_id,city_id) VALUES (?,?,?,?,?)"
    );
    stmt.run(bank, comment, amount, purchase_id,city_id);
    stmt.finalize();
    callback("true");
  });
};
...
update_payment = function (date, purchase_id, callback) {
  db.connection.serialize(function () {
    var stmt = db.connection.prepare(
      "UPDATE payment SET date = ? WHERE payment_id = ?"
    );
...
update_payment_item = function (bank, comment, amount, payment, city, callback) {
  ...
  db.connection.serialize(function () {
    var stmt = db.connection.prepare(
      "UPDATE payment_item SET bank=?, comment = ?, amount=?, city_id = ? WHERE payment_item_id = ?"
    );
...
function valid_purchase(party, date, bank, comment, amount, purchase_id,city_id) {
  ...
  db.connection.all(sql, [purchase_id], (err, rows) => {
    ...
    if (rows.length > 0) {
      add_purchase_item(bank, comment, amount, purchase_id,city_id, function (add_py) {
...
insert_post_payment = function (amount1, purchase_id, callback) {
  db.connection.serialize(function () {
    var stmt = db.connection.prepare(
      `INSERT INTO invoice_pay (paid, payment_id) 
      SELECT ?,?
      WHERE NOT EXISTS (Select payment_id From invoice_pay WHERE payment_id =?) LIMIT 1;`
    );
...
delete_post_payment = function (purchase_id, callback) {
  db.connection.serialize(function () {
    var stmt = db.connection.prepare(
      "DELETE from invoice_pay where payment_id=?"
    );
...
function show_purchase() {
  ...
  rows = [{ bank_name: "", comment: "", city: "", amount: "" }];
  rowss = [{ bank_name: "", comment: "", city: "", amount: "" }];
  ...
  $("#my-table").DataTable({
    ...
    data: rowss,
    columns: [{ data: "bank_name" }, { data: "comment" }, { data: "city" }, { data: "amount" }],
    bFilter: false,
    bJQueryUI: true,
  ...
  var sql = "SELECT payment_id AS id FROM payment";
  ...
  db.connection.all(sql, [], (err, rows) => {
    ...
    $("#purchase_bank").append(
      $("<option>").text("Select the bank").attr("value", "")
    );
    for (let i = 0; i < rows.length; i++) {
      $("#purchase_bank").append(
        $("<option>").text(rows[i].name).attr("value", rows[i].id)
  ...
  db.connection.all(sql, [], (err, rows) => {
    ...
    $("#purchase_city").append(
        $("<option>").text("Select the city").attr("value", "")
    );
    for (let i = 0; i < rows.length; i++) {
      $("#purchase_city").append(
          $("<option>").text(rows[i].name).attr("value", rows[i].id)
  ...
  var sql = "Select payment_id from payment where status = 0";
  db.connection.all(sql, [], (err, rows) => {
    if (err) {
      throw err;
    }
...
function show_purchase_added() {
  ...
  var sql =
    "SELECT b.bank_name ,p.comment,p.amount, c.city_name FROM bank_group b left JOIN payment_item p on p.bank = b.bank_id LEFT JOIN city_group c on c.city_id = p.city_id where payment_id=?";
  db.connection.all(sql, [purchase_id], (err, rows) => {
    if (err) {
      throw err;
    }
    ...
    rowss = [{ bank_name: "", comment: "", amount: "", city_name: "" }];
...
function show_edit_detail_purchase(purchase_id) {
  ...
  var sql =
    "SELECT payment_item_id as id, amount as name FROM payment_item, payment where payment_item.payment_id = payment.payment_id and payment_item.payment_id=?";
  db.connection.all(sql, [purchase_id], (err, rows) => {
    if (err) {
      throw err;
    }
    ...
    if (rows.length > 0) {
      ...
      var sql = "SELECT * FROM payment_item where payment_item_id=?";
